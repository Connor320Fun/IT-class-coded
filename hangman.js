...
let hmWords = ['apple','banana','orange','grape','strawberry','pineapple','watermelon','lemon','avocado','mango','pear','peach','plum','kiwi','papaya','apricot','coconut','blueberry','raspberry','blackberry','elephant','giraffe','dolphin','kangaroo','hippopotamus','rhinoceros','alligator','crocodile','chimpanzee','gorilla','penguin','owl','eagle','falcon','sparrow','butterfly','beetle','octopus','squid','whale','shark','dolphin','seal','otter','rabbit','squirrel','fox','wolf','lion','tiger','bear','zebra','horse','donkey','camel','llama','python','javascript','typescript','ruby','perl','go','rust','java','csharp','cplus','coding','developer','programmer','algorithm','function','variable','constant','object','array','string','number','boolean','computer','keyboard','monitor','printer','mouse','internet','network','server','client','backend','frontend','database','sql','nosql','github','gitlab','netlify','react','angular','vue','svelte','webpack','babel','docker','kubernetes','api','endpoint','cache','thread','process','compile','debug','deploy','build','test','hangman','mystery','puzzle','challenge','adventure','treasure','island','castle','dungeon','forest','mountain','valley','river','ocean','desert','canyon','continent','country','city','village','harbor','bridge','tunnel','monument','museum','library','school','university','hospital','station','airport','biology','chemistry','physics','astronomy','geology','ecology','meteorology','psychology','sociology','philosophy','history','geography','economics','politics','technology','innovation','science','research','experiment','discovery','invention','creation','imagination','vision','strategy'];
let hmWord = '';
let hmDisplay = [];
let hmAttempts = 6;
...
haOwnerDebugBtn && haOwnerDebugBtn.addEventListener('click', ()=>{ window.haOwnerDebugMode = !window.haOwnerDebugMode; alert('Owner debug mode ' + (window.haOwnerDebugMode ? 'ENABLED' : 'DISABLED')); console.log('Owner debug toggled', window.haOwnerDebugMode); });
